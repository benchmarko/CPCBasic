...
Canvas.prototype = {
	...
	fill: function (iFill) {
		...
		while (aPixels.length) {
			//console.log(aPixels.length); //TTT
			oPixel = aPixels.pop();
			y1 = oPixel.y;
	...
	fill_ok3: function (iFill) {
		var that = this,
			...
			iLineHeight = this.aModeData[this.iMode].iLineHeight,
			aPixels = [],
			oPixel, y1, bSpanLeft, bSpanRight, p1, p2, p3,
			fnIsStopPen = function (p) {
				return p === iFill || p === iGPen;
			},
			fnIsNotInWindow = function (x, y) {
		...
		while (aPixels.length) {
			...
			while (y1 <= (that.iHeight - 1 - that.yBottom) && !fnIsStopPen(p1)) {
				/ *
				if (fnIsNotInWindow(oPixel.x, y1)) { //TTT
					break;
				}
				* /
				this.setSubPixels(oPixel.x, y1, iFill, 0);

				p2 = this.testSubPixel(oPixel.x - iLineWidth, y1);
				if (!bSpanLeft && oPixel.x > this.xLeft && !fnIsStopPen(p2)) {
					aPixels.push({
						x: oPixel.x - iLineWidth,
						y: y1
					});
					bSpanLeft = true;
				} else if (bSpanLeft && ((oPixel.x - iLineWidth >= this.xLeft) || fnIsStopPen(p2))) {
					bSpanLeft = false;
				}

				p3 = this.testSubPixel(oPixel.x + iLineWidth, y1);
				if (!bSpanRight && oPixel.x <= this.xRight && !fnIsStopPen(p3)) {
					aPixels.push({
						x: oPixel.x + iLineWidth,
						y: y1
					});
					bSpanRight = true;
				} else if (bSpanRight && ((oPixel.x > this.xRight - iLineWidth) || fnIsStopPen(p3))) {
					bSpanRight = false;
				}
				y1 += iLineHeight;
				p1 = this.testSubPixel(oPixel.x, y1);
	...
	*/
	...
	fill_ok2: function (iFill) {
		...
		while (aPixels.length) {
			oPixel = aPixels.pop();
			y1 = oPixel.y;
			p1 = this.testPixel(oPixel.x, y1);
			while (y1 >= this.yBottom && !fnIsStopPen(p1)) {
				y1 -= iLineHeight;
				p1 = this.testPixel(oPixel.x, y1);
			}
			y1 += iLineHeight;
			...
			bSpanRight = false;
			p1 = this.testPixel(oPixel.x, y1);
			while (y1 < this.yTop && !fnIsStopPen(p1)) {
				this.setPixel(oPixel.x, y1, iFill, 0);

				p2 = this.testPixel(oPixel.x - iLineWidth, y1);
				if (!bSpanLeft && oPixel.x > this.xLeft && !fnIsStopPen(p2)) {
					aPixels.push({
						x: oPixel.x - iLineWidth,
						y: y1
				...
				p3 = this.testPixel(oPixel.x + iLineWidth, y1);
				if (!bSpanRight && oPixel.x <= this.xRight && !fnIsStopPen(p3)) {
					aPixels.push({
						x: oPixel.x + iLineWidth,
						y: y1
	...
	*/
	...
	fill_ok1: function (iFill) {
		var xPos = this.xPos,
			...
			iLineHeight = this.aModeData[this.iMode].iLineHeight,
			pixels = [],
			temp, y1, spanLeft, spanRight, targetColor, p1, p2, p3;
		...
		while (pixels.length) {
			temp = pixels.pop();
			y1 = temp.y;
			p1 = this.testPixel(temp.x, y1);
			while (y1 >= this.yBottom && p1 === targetColor) {
				y1 -= iLineHeight;
				p1 = this.testPixel(temp.x, y1);
			}
			y1 += iLineHeight;
			...
			spanRight = false;
			p1 = this.testPixel(temp.x, y1);
			while (y1 < this.yTop && p1 === targetColor) {
				this.setPixel(temp.x, y1, iFill, 0);

				p2 = this.testPixel(temp.x - iLineWidth, y1);
				if (!spanLeft && temp.x > this.xLeft && p2 === targetColor) {
					pixels.push({
						x: temp.x - iLineWidth,
						y: y1
					});
					spanLeft = true;
				} else if (spanLeft && temp.x - iLineWidth >= this.xLeft && p2 !== targetColor) {
					spanLeft = false;
				}

				p3 = this.testPixel(temp.x + iLineWidth, y1);
				if (!spanRight && temp.x <= this.xRight && p3 === targetColor) {
					pixels.push({
						x: temp.x + iLineWidth,
						y: y1
					});
					spanRight = true;
				} else if (spanRight && temp.x > this.xRight - iLineWidth && p3 !== targetColor) {
					spanRight = false;
				}
				y1 += iLineHeight;
				p1 = this.testPixel(temp.x, y1);
	...
	*/
	...
	floodFill: function (Bitmap bmp, Point pt, Color targetColor, Color replacementColor) {
	...
	*/
	...
	fillTest1: function (iFill) {
		...
		//idx = x + iWidth * y
		// for (y = yPos; y < this.yTop; y += 1) {
		// 	idx = xPos + iWidth * y;
		// 	iP1 = dataset8[idx];
		// 	if (iP1 === iPen || iP1 === iGPen) {
		// 		break;
		// 	}
		// 	dataset8[idx] = iPen;
		// }
		...
		for (y = yPos; y <= this.yTop; y += iLineHeight) {
			iP1 = this.testPixel(xPos, y);
			if (iP1 === iFill || iP1 === iGPen) {
				break;
			}
			this.setPixel(xPos, y, iFill, 0);
		}
		y1 = y;

		for (y = yPos - iLineHeight; y >= this.yBottom; y -= iLineHeight) {
			iP1 = this.testPixel(xPos, y);
			if (iP1 === iFill || iP1 === iGPen) {
				break;
			}
		...
		// right side
		aBuf.push([y, y1, x + iLineWidth, 1]); // eslint-disable-line array-element-newline
		...
		this.setNeedUpdate(0, 0, this.iHeight, this.iWidth); // TTT
	},
	*/
